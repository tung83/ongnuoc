...
function slider()
{
	...
	{
		$sInsert="insert into slide(ind,active,lnk,title";
		$sInsert.=") values($ind,$active,'$lnk','$title')";
		...
		if(checkImg($file)==true)
		{
			...
			$rexobj -> resizeImage(544,300, 'exact');
			$rexobj->saveImage(myPath.$file,100);
			mysql_query("update slide set img='$file' where id=$recent");	
		}
		if($test)
		{
	...
	{
		$sUpdate="update slide set active=$active,ind=$ind,lnk='$lnk',title='$title'";
		if(checkImg($file)==true)
		{
			move_uploaded_file($_FILES["file"]["tmp_name"],myPath.$file);
			$rexobj = new resize(myPath.$file);
	...
	{
		$sql="select * from slide where id=".$_POST["idLoad"];
		$tabEdit=mysql_query($sql);
		$rowEdit=mysql_fetch_object($tabEdit);
	}
	if(isset($_POST["Del"])&&$_POST["Del"]==1)
	{
		$sDelete="delete from slide where id=".$_POST["idLoad"];
		 ...
		 <div class="col-lg-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
					';
	$s="select * from slide order by id desc";
	$tab=mysql_query($s);
