...
function price()
{
	...
	{
		$sInsert="insert into price(title,sum,content,active,dates,meta_keyword,meta_description";
		...
		if(checkImg($file)==true)
		{
			...
			mysql_query("update price set img='$file' where id=$recent");	
		}
		if($test)
		{
	...
	{
		$sUpdate="update price set title='$title',active=$active";
	...
	{
		$sql="select * from price where id=".$_POST["idLoad"];
		$tabEdit=mysql_query($sql);
		$rowEdit=mysql_fetch_object($tabEdit);
	}
	if(isset($_POST["Del"])&&$_POST["Del"]==1)
	{
		$sDelete="delete from price where id=".$_POST["idLoad"];
	...
	<div class="row">
		<div class="col-lg-12">
			<ol class="breadcrumb">
				<li>
					<i class="fa fa-dashboard"></i> <a href="main.php?act=price">Bảng giá</a>
				</li>
			</ol>
		</div>
	</div>';
		 ...
		 <div class="col-lg-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
					';
	$s="select * from price order by id desc";
	$tab=mysql_query($s);
	...
	$str.=ad_paging($lim,$count,'main.php?act=price&',$page);
	$str.='
			</div>
		</div>
		<!-- Row -->
