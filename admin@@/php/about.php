...
function about()
{
	...
	{
		$sInsert="insert into about(title,sum,content,active,dates,meta_keyword,meta_description";
		$sInsert.=") values('$title','$sum','$content',$active,now(),'$meta_keyword','$meta_description')";
		...
		if(checkImg($file)==true)
		{
			...
			mysql_query("update about set img='$file' where id=$recent");	
		}
		if($test)
		{
	...
	{
		$sUpdate="update about set title='$title',active=$active";
	...
	{
		$sql="select * from about where id=".$_POST["idLoad"];
		$tabEdit=mysql_query($sql);
		$rowEdit=mysql_fetch_object($tabEdit);
	}
	if(isset($_POST["Del"])&&$_POST["Del"]==1)
	{
		$sDelete="delete from about where id=".$_POST["idLoad"];
	...
	<div class="row">
		<div class="col-lg-12">
			<ol class="breadcrumb">
				<li>
					<i class="fa fa-dashboard"></i> <a href="main.php?act=about">About Us</a>
				</li>
			</ol>
		</div>
	</div>';
		 ...
		 <div class="col-lg-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
					';
	$s="select * from about order by id desc";
	$tab=mysql_query($s);
	...
	$str.=ad_paging($lim,$count,'main.php?act=about&',$page);
	$str.='
			</div>
		</div>
		<!-- Row -->
