...
{
    if(isset($_GET['pId']))	return news();
    else return news_cate();	
}
function news_cate(){
    $msg='';
    $act='news';
...
function news()
{
    ...
    $table='news';
    $pId=intval($_GET['pId']);
    $cate=mysql_query("select id,title from news_cate where id=$pId");
	...
	if(isset($_POST["addNew"]))
	{
		$sInsert="insert into news(title,sum,content,active,dates,meta_keyword,meta_description,pId";
		...
		if(checkImg($file)==true)
		{
			...
			mysql_query("update news set img='$file' where id=$recent");	
		}
		if($test)
		{
			header("location:".$_SERVER['REQUEST_URI'],true);
	...
	if(isset($_POST["update"]))
	{
		$sUpdate="update news set title='$title',active=$active";
	...
	<div class="row">
		<div class="col-lg-12">
			<ol class="breadcrumb">
				<li>
					<i class="fa fa-dashboard"></i> <a href="main.php?act=news">Danh mục tin tức</a>
				</li>
                <li class="active">
                    '.$cate->title.'
                </li>
		 ...
		 <div class="col-lg-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
					';
	$s="select * from news where pId=$pId order by id desc";
	$tab=mysql_query($s);
	$count=mysql_num_rows($tab);
	$page=isset($_GET["page"])?intval($_GET["page"]):1;
	$lim=10;
	...
	$str.=ad_paging($lim,$count,'main.php?act=news&',$page);
	$str.='
			</div>
		</div>
		<!-- Row -->
